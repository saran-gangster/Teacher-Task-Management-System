...
def create_table(cursor,console):
    ...
    with console.status("[bold green]Checking/Creating Class Table..") as status:
        sleep(2)
        query = "CREATE TABLE if not exists Class(class_id int(5) PRIMARY KEY,class int(2),division varchar(3),class_teacher varchar(20),no_of_students int(3),subject1 varchar(20),subject2 varchar(20),subject3 varchar(20),subject4 varchar(20),subject5 varchar(20));"
    ...
    with console.status("[bold green]Checking/Creating Teachers Table..") as status:
        sleep(2)
        query = "CREATE TABLE if not exists Teachers(teachers_id int(5)PRIMARY KEY,teachers_name varchar(30) NOT NULL,department varchar(20),date_of_joining date,gender varchar(2),address varchar(50), ph_no varchar(11),class_id int(5));"
        cursor.execute(query)
